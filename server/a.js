...
app.get('/api/route', async (req, res) => {
  ...
  if (!placeName || isNaN(distanceInKm)) {
    return res
      .status(400)
      .json({ error: 'Place name and valid distance are required' })
  }
  ...
  try {
    ...
    for (const place of placesResponse.data.results) {
      ...
      if (
        location.lat !== startingCoordinates.lat ||
        location.lng !== startingCoordinates.lng
      ) {
    ...
    if (directionsToStart.data.status !== 'OK') {
      return res
        .status(404)
        .json({ error: 'No route found back to starting point' })
    }
    ...
    const routeToStart =
      directionsToStart.data.routes.find((route) => {
        return (
          ...
          routeToDestination.overview_polyline.points
        )
